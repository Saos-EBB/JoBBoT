...
- `matched` and `uncertain` jobs live in `data/jobs/sicher/` and
  `data/jobs/unsicher/` respectively; every other status stays in the base
  `data/jobs/` dir. `JsonStore.save()` moves the file between these when
  status changes — don't assume a job's path is stable across a status update.
